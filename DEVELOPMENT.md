# Evolution API - Development Guide

This document provides comprehensive guidelines for developers (human or AI) working with the Evolution API codebase.

## Project Overview

**Evolution API** is a powerful, production-ready, multi-tenant REST API for WhatsApp communication. Built with **Node.js 20+**, **TypeScript 5+**, and **Express.js**, it supports multiple WhatsApp providers and provides extensive integrations with chatbots, CRM systems, and messaging platforms.
...
- **`src/`** – TypeScript source code with modular architecture
  ...
  - `api/integrations/` – External service integrations
    ...
    - `chatbot/` – AI/Bot integrations (EvolutionBot, OpenAI, Dify, Typebot, Chatwoot, Flowise, N8N, EvoAI)
...
- **Channel Integrations** (`src/api/integrations/channel/`): Baileys, Meta Business API, and Evolution API with unified interface. Connection lifecycle management per instance with automatic reconnection.
- **Chatbot Integrations** (`src/api/integrations/chatbot/`): EvolutionBot, Chatwoot, Typebot, OpenAI, Dify, Flowise, N8N, EvoAI.
- **Event Integrations** (`src/api/integrations/event/`): WebSocket, RabbitMQ, Amazon SQS, NATS, Pusher.
- **Storage Integrations** (`src/api/integrations/storage/`): AWS S3, MinIO.
