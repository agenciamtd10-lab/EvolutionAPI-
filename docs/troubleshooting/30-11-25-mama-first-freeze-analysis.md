...
## 1) Incident Recap
- **Nov 30 08:33 WIB:** Instance froze. `connectionStatus=open` but `updatedAt` stale. Logs show `PrismaClientValidationError: Invalid this.cache.delete()` in `messages.update` (stack at `/evolution/dist/main.js:227:33183`). PENDING greetings (malformed `{"conversation": ...}`) were present before the freeze.
...
1. Prisma/cache-delete bug in `messages.update` caused crash loops around the restart, breaking Chatwoot webhook processing and history sync; many incoming/outgoing events persisted but failed to sync, leaving `chatwootMessageId` null.
2. PENDING/interactiveMessage payloads remain unsupported/poison; they coincide with error bursts and remain unsynced.
3. Massive Prospek-side duplication on Nov 30 (10 conversations per contact at identical timestamps) likely from Prospek automation, inflating counts and forcing Evolution to create/reopen repeatedly.
...
1. **Backfill/Replay Dec 1 gap:** Extract unsynced Evolution rows (Dec 1, `chatwootMessageId IS NULL`) and replay to Prospek, or at least archive the CSV for manual import.
2. **Stabilize sync path:** Patch `cache.delete` misuse in `messages.update` and guard Chatwoot webhook processing; fix PENDING/interactiveMessage handling so they build a full envelope and do not poison the queue.
3. **Dedupe containment:** In Prospek, block new conversation creation for inbox 71 if a convo exists in last 24h for the contact; merge the 10x clones from Nov 30 (contacts starting with IDs 162753–162776 and specific numbers above).
4. **Monitoring:** Alert on PrismaClientValidationError rate, PENDING count >0, unsynced rate, and conversation create spikes.
...
## 6) Tooling (logs & DB) — SSOT
- Log pulls (Azure Log Analytics): see `docs/troubleshooting/logs-howto.md` for subscription/resource IDs and example commands. Key workspace: `3aaf7750-6587-4814-99ce-72558b7dde41`. Example query (Dec 1 UTC):
  - Evolution: save to `docs/troubleshooting/logs-evolution-prod-2025-12-01.json`
  - Prospek: save to `docs/troubleshooting/logs-prospek-prod-2025-12-01.json`
